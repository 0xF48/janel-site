...
export function BookLinksRow({ book }: { book: Book }) {
	return <div className='flex flex-row gap-10 md flex-wrap px-10 justify-between'>
		{book.audiobook_link ? <a target='_blank' href={book.audiobook_link} className="transition-colors text-cyan-300 hover:text-cyan-500 font-mono text-md mt-2 ml-4 flex flex-row items-center gap-2 hover:underline">Audiobook<HeadphonesIcon size={18} strokeWidth={2} /></a> : null}
